...
- **Dark theme first.** Argos is a cyberpunk-themed tactical tool. Dark mode is the primary and default experience. Light mode is supported as a secondary option for daylight field operations (spec 004).
...
- **Dark mode is default.** Argos is dark-theme-first. All new components must look correct in dark mode first. Light mode support is secondary — components may gracefully degrade in light mode during incremental migration. Do not assume light backgrounds in any component that hasn't been explicitly upgraded.
