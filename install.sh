...
clone_repository() {
    ...
    # Clone repository
    git clone "${REPO_URL}" "${PROJECT_DIR}" &> "${LOG_FILE}" || error "Failed to clone repository"
...
install_hackrf_container() {
    log "Setting up HackRF Docker container..."
    
    # Check if OpenWebRX image exists
    local image_file="${PROJECT_DIR}/docker-images/openwebrx-hackrf-only-v2.tar"
    
    if [[ -f "$image_file" ]]; then
        log "Loading HackRF container from local image..."
        sudo docker load < "$image_file" &> "${LOG_FILE}" || error "Failed to load Docker image"
    else
        warn "HackRF Docker image not found. Building from Dockerfile..."
        if [[ -f "${PROJECT_DIR}/docker/Dockerfile" ]]; then
            cd "${PROJECT_DIR}/docker"
            sudo docker build -t hackrf:latest . &> "${LOG_FILE}" || error "Failed to build Docker image"
            cd "${PROJECT_DIR}"
        else
            warn "No Docker configuration found. HackRF features may not work."
        fi
    fi
    
    log "HackRF container setup complete"
}
