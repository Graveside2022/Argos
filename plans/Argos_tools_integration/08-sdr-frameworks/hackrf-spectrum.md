...
#
# HackRF One (USB)
#   -> hackrf_sweep (C binary, receive-only sweep mode)
#   -> hackrf_emitter/ (Python service, parses sweep output)
...
**ARM64 NATIVE** - The hackrf tools (`hackrf_sweep`, `hackrf_info`, `hackrf_transfer`) and libhackrf are available as pre-built ARM64 packages in the Kali Linux repositories. The Python hackrf_emitter service and all SvelteKit frontend components are architecture-independent. No compilation required for standard installation.
