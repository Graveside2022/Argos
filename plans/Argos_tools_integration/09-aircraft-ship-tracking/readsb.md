...
| **Native Install**   | YES       | Builds from source on aarch64 with `make`; all deps (`librtlsdr-dev`, `libprotobuf-c-dev`) in Kali 2025.4 repos            |
...
readsb uses Protocol Buffers (protobuf) for internal data representation, resulting in more efficient data handling and reduced CPU overhead compared to dump1090's JSON-based internal format. It maintains full API compatibility with dump1090-fa, meaning all tools and services that consume dump1090 data work seamlessly with readsb.
...
- Reduced CPU usage through protobuf-based internal data representation
...
- `protobuf-c` library for protobuf support
...
| Performance on RPi5   | :white_check_mark: Excellent -- lower CPU usage than dump1090-fa due to protobuf optimizations, ~1-3% CPU on Cortex-A76, <25MB RAM |

### RPi5-Specific Notes

- readsb is specifically optimized for ARM processors and performs better than dump1090-fa on Raspberry Pi hardware
- The protobuf-based internal data format reduces memory allocations and CPU cycles
...
- All build dependencies (`libprotobuf-c-dev`, `librtlsdr-dev`) are available in Kali ARM64 repositories
