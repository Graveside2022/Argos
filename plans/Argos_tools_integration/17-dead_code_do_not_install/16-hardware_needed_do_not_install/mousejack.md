...
> **NOTE:** The `nrf-research-firmware` is a git submodule within the mousejack repo — clone with `--recursive`. Flash script is `prog/usb-flasher/usb-flash.py` (not the previously documented nrf24lu1p scripts which do not exist).
...
# Clone MouseJack research repo (--recursive pulls nrf-research-firmware submodule)
RUN git clone --recursive https://github.com/BastilleResearch/mousejack.git /opt/mousejack
...
git clone --recursive https://github.com/BastilleResearch/mousejack.git
cd mousejack
...
sudo python3 prog/usb-flasher/usb-flash.py bin/dongle.bin
