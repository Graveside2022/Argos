...
- **GitHub**: https://github.com/martinmarinov/TempestSDR (original) / https://github.com/eried/Research/tree/master/HackRF/TempestSDR (demos only, not a code fork)
- **Language**: Java (GUI/DSP), C (JNI native libraries for SDR hardware access)
- **License**: GPL-3.0

---

> **NOTE**: The original martinmarinov/TempestSDR repo has no dedicated HackRF plugin. Available plugins are: `TSDRPlugin_RawFile`, `TSDRPlugin_ExtIO`, `TSDRPlugin_Mirics`, `TSDRPlugin_SdrPlay`, `TSDRPlugin_UHD`. HackRF access may be possible through the ExtIO interface. The eried/Research repo contains only demo videos, not a code fork. Build steps below reference the actual plugin directories from the original repo.
...
# Build native JNI libraries (actual plugins in repo)
# NOTE: No dedicated HackRF plugin exists; HackRF access is via ExtIO interface
RUN cd JavaGUI && \
    mkdir -p libs && \
    cd ../TSDRPlugin_RawFile && make && \
    cd ../TSDRPlugin_ExtIO && make && \
    cd ../TSDRPlugin_Mirics && make && \
    cd ../TSDRPlugin_UHD && make || true
...
# Actual plugins in repo: RawFile, ExtIO, Mirics, SdrPlay, UHD
# NOTE: No dedicated HackRF plugin exists; HackRF access is via ExtIO interface
cd TSDRPlugin_RawFile && make && cd ..
cd TSDRPlugin_ExtIO && make && cd ..
cd TSDRPlugin_Mirics && make && cd ..
cd TSDRPlugin_UHD && make || cd ..
