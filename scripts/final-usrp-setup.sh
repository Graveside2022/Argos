...
echo "🔧 Final USRP B205 Mini Setup for OpenWebRX"
echo "=============================================="
...
if curl -s http://100.79.154.94:5173/viewspectrum > /dev/null; then
    echo "   ✅ Mission card URL is accessible"  
else
    echo "   ❌ Mission card URL failed"
    exit 1
fi

# Check for technical issues
if curl -s http://localhost:8073 | grep -q "technical issues"; then
    echo "   ⚠️  OpenWebRX shows technical issues - checking logs..."
    docker logs openwebrx-usrp 2>&1 | tail -5
else
    echo "   ✅ No technical issues detected"
fi
