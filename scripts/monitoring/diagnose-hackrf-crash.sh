...
if lsusb | grep -q "1d50:6089"; then
    echo "   [PASS] HackRF One detected on USB"
    hackrf_info 2>&1 | grep -E "Serial|Firmware|Part ID" | sed 's/^/   /'
else
    echo "   [FAIL] No HackRF detected!"
...
if swapon --show | grep -q "/swapfile"; then
    ...
    echo "   [FAIL] No swap configured (run: sudo ./scripts/setup-swap.sh)"
fi
echo ""
...
case $EXIT_CODE in
    0)
        ...
        ;;
    124)
        echo "   [PASS] Test timed out (expected for continuous sweep)"
        ;;
    *)
