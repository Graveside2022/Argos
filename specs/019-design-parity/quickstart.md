...
4. **`--palantir-*` migration approach**: Full find-replace of all 254 `var(--palantir-*)` references across 32 files with direct Lunaris tokens. The bridge file's non-palantir tokens (`--space-*`, `--text-*`, etc.) MUST be migrated to `app.css` BEFORE the `:root` block is deleted. Phase 7 uses 3 atomic commits for rollback safety.
