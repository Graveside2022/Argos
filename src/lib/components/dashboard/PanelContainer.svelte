...
<style>
	...
	.panel-container[data-position='top'] .drag-handle {
		border-top: 1px solid var(--palantir-border-subtle);
	}

	.panel-container[data-position='bottom'] .drag-handle {
		border-bottom: 1px solid var(--palantir-border-subtle);
	}
	...
	.drag-handle:focus {
		outline: none;
		box-shadow: inset 0 0 0 2px var(--palantir-accent);
	}

	.drag-indicator {
		width: 40px;
		height: 3px;
		background: var(--palantir-border-default);
		border-radius: 2px;
		transition: background 0.15s ease;
	}
