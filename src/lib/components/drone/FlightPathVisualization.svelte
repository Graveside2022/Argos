<script lang="ts">
	...
	function updateVisualization() {
		...
		const pathPoints: [number, number][] = flightPath.map(
		...
		if (showAltitude) {
			...
			for (let i = 1; i < flightPath.length; i++) {
				const L = (window as unknown as { L: LeafletConstructor }).L;
				const segment = L.polyline([pathPoints[i - 1], pathPoints[i]], {
        ...
        `);
				...
				if (capture.strongestSignal && signalLayer) {
					const bearing = calculateBearing(capture.position, {
						lat: capture.strongestSignal.lat,
						lon: capture.strongestSignal.lon
					});

					const arrow = createDirectionArrow(
						capture.position,
						bearing,
						capture.strongestSignal.power
					);
	...
	function createAltitudeMarkers(): LeafletLayer[] {
        ...
        `,
			...
			markers.push(L.marker([point.lat, point.lon], { icon: marker }));
		}
	...
	function addFlightMarkers() {
      ...
      `,
		...
		const startMarker = L.marker([flightPath[0].lat, flightPath[0].lon], { icon: startIcon })
			.bindPopup(`
    ...
    `);
      ...
      `,
		...
		const endMarker = L.marker([endPoint.lat, endPoint.lon], { icon: endIcon }).bindPopup(`
