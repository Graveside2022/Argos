<script lang="ts">
	...
	function clearWaypoints() {
		...
		if (map) {
			map.eachLayer((layer: LeafletLayer) => {
				if (layer.options?.className === 'mission-path' || layer.options?.draggable) {
					map.removeLayer(layer);
				}
			});
		}
	...
	function createAndStartMission() {
		...
		const mission = createMission(missionName, missionType, plannedWaypoints, undefined, {
			signalThreshold,
			scanInterval,
		...
		<div class="active-mission" transition:slide>
			<div class="mission-header">
				...
				<span
					...
					class:active={missionActive}
					class:paused={missionPaused}
				>
			...
			<div class="mission-stats">
				...
				<div class="stat">
					...
					<span class="value"
						>{formatDuration($activeMission.statistics.totalDuration)}</span
					>
				</div>
				...
				<div class="stat">
					...
					<span class="value"
						>{($activeMission.statistics.totalDistance / 1000).toFixed(1)} km</span
					>
				</div>
			</div>
		...
		<div class="mission-planner" transition:slide>
			...
			<div class="planner-content">
				...
				<div class="waypoints-section">
					...
					{#if drawingMode}
						<div class="drawing-hint">Click on the map to add waypoints</div>
					{/if}
					...
					<div class="waypoints-list">
						{#each plannedWaypoints as waypoint, index}
							<div
								class="waypoint-item"
								class:selected={selectedWaypoint?.id === waypoint.id}
							>
