<script lang="ts">
	...
	let detectedSignals: SignalDetection[] = $state([]);
	...
	onMount(() => {
		const signalsUnsubscribe = hackrfService.detectedSignals.subscribe(
			(signals: SignalDetection[]) => {
				detectedSignals = signals.slice(-5);
			}
		);
		return () => signalsUnsubscribe();
	});
</script>
...
>
	<div class="flex items-center mb-6">
		<div
			class="p-3 bg-gradient-to-br from-purple-500/20 to-purple-500/10 rounded-xl mr-4 border border-purple-400/20 group-hover:border-purple-400/40 group-hover:shadow-lg group-hover:shadow-purple-400/20 transition-all duration-300"
		>
			<svg
				class="w-6 h-6 text-purple-400 group-hover:scale-110 transition-transform duration-300"
				fill="currentColor"
				viewBox="0 0 20 20"
			>
				<path
	...
	<div class="space-y-6">
		<!-- External Tools -->
		<div class="space-y-3">
			<button onclick={launchOpenWebRX} class="saasfly-btn saasfly-btn-outline w-full group">
				<svg
					class="w-4 h-4 group-hover:text-neon-cyan transition-colors duration-200"
					fill="currentColor"
					viewBox="0 0 20 20"
				>
					<path
			...
			<button
				onclick={launchSpectrumAnalyzer}
				class="saasfly-btn saasfly-btn-outline w-full group"
			>
				<svg
					class="w-4 h-4 group-hover:text-purple-400 transition-colors duration-200"
					fill="currentColor"
					viewBox="0 0 20 20"
				>
		...
		{#if detectedSignals.length > 0}
			<div>
				<h4 class="text-sm font-medium text-text-muted mb-3 uppercase tracking-wide">
					Recent Signals
				</h4>
				<div class="space-y-2 max-h-[150px] overflow-y-auto">
					{#each detectedSignals as signal}
						<div
							class="p-3 bg-bg-input/40 rounded-lg border border-border-primary/30 hover:border-purple-400/30 transition-all duration-200"
						>
							<div class="flex justify-between items-center">
								<span class="font-mono text-sm"
									>{(signal.frequency / 1e6).toFixed(2)} MHz</span
								>
