...
<div class="glass-panel rounded-xl p-6">
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
		<div class="space-y-2">
			<label
				for="freqStart"
				class="block font-mono text-xs text-text-muted uppercase tracking-wider"
				>Start (MHz)</label
			>
			<input
				id="freqStart"
		...
		<div class="flex items-end">
			<button onclick={addFrequencyRange} class="saasfly-button-primary w-full">
	...
	<div class="space-y-3">
		{#each $frequencyRanges as range (range.id)}
			<div
				class="glass-panel-light rounded-lg p-4 flex items-center justify-between hover:border-accent-primary/40 transition-all duration-300 group"
			>
				...
				<button
					onclick={() => removeFrequencyRange(range.id)}
