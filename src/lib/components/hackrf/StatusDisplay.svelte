<script lang="ts">
	...
	let currentFrequency = $state('0.0');
	let cycleStatus = $state('Disconnected');
	let signalStrength = $state(-100);
	let elapsedTime = $state('00:00');
	let isConnected = $state(false);
	let _detectedSignals: SignalDetection[] = $state([]);
	...
	onMount(() => {
		const statusUnsubscribe = hackrfService.status.subscribe((status: HackRFStatus) => {
		...
		const spectrumUnsubscribe = hackrfService.spectrumData.subscribe(
