<script lang="ts">
	...
	export let availableSources: { interface: string; name: string; hasMonitorMode: boolean }[] = [];
	...
	function confirmSelection() {
		if (selectedSource) {
			dispatch('select', { interface: selectedSource });
			close();
		}
	}
...
{#if isOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center">
		...
		<!-- Modal -->
		<div class="relative z-10 bg-bg-primary border border-border-primary rounded-lg p-6 max-w-md w-full mx-4">
			<h2 class="text-xl font-bold text-text-primary mb-4">
				Select WiFi Data Source
			</h2>
				...
				{#if availableSources.length === 0}
					<div class="text-text-secondary text-center py-8">
						<svg class="w-12 h-12 mx-auto mb-4 opacity-50" fill="currentColor" viewBox="0 0 24 24">
							<path d="M23 11l-2.5-2.5-1.42 1.42L20.17 11H16c-.55 0-1 .45-1 1s.45 1 1 1h4.17l-1.09 1.08 1.42 1.42L23 13v-2m-10-2V3.5C13 2.67 12.33 2 11.5 2S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5v-2l-8-5z"/>
						</svg>
					...
					{#each availableSources as source}
						...
						>
							<input 
								...
								on:change={() => selectSource(source.interface)}
								class="mr-3"
							/>
							<div class="flex-1">
