<script lang="ts">
  ...
  async function toggleRFDetection() {
    ...
    if (!isRFEnabled) {
      const hasSelectedFrequencies = Object.values(frequencyBands).some(v => v) || targetFrequency;
      if (!hasSelectedFrequencies) {
        alert('Please select at least one frequency band or enter a custom frequency before starting the RF scan.');
        isProcessing = false;
        return;
      }
    }
    ...
    try {
      if (!isRFEnabled) {
        ...
        deviceInfo = status.deviceInfo?.usrp?.info || 'USRP B205 mini';
        ...
        if (status.isRunning) {
          ...
          if (response.ok) {
            ...
            console.log('USRP sweep started successfully with frequencies:', frequencies);
            
            // Subscribe to spectrum data
            if (!spectrumUnsubscribe) {
              spectrumUnsubscribe = spectrumData.subscribe((data) => {
                console.log('[AirSignal] Spectrum data subscription triggered:', data);
  ...
  function processSpectrumData(data: any) {
    ...
    if (!data.power_levels || !Array.isArray(data.power_levels) || data.power_levels.length === 0) {
    ...
    console.log(`Detected signal: ${peakFreq.toFixed(1)} MHz @ ${maxPower.toFixed(1)} dBm - ${signalType}`);
        ...
        >
          <div class="freq-icon">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
              <path d="M1 9l2-2v8h-2zm4-1l2-2v10h-2zm4-1l2-2v12h-2zm16-4l-7 7-4.5-4.5L5 14l-2-2L13.5 1.5 19 7l5-5z"/>
            </svg>
          </div>
          <div class="freq-info">
