<script lang="ts">
	...
	function toggleSort(column: SortColumn) {
		if (sortColumn === column) {
			...
			sortColumn = column;
			sortDirection = column === 'signal' ? 'asc' : 'asc';
		}
		updateFilteredDevices();
	}

	function getDeviceSignal(device: KismetDevice): number {
		return typeof device.signal === 'object'
			? device.signal?.last_signal || -100
			: (device.signal as number) || -100;
	}

	function sortDevices(deviceList: KismetDevice[]): KismetDevice[] {
		return [...deviceList].sort((a, b) => {
			let cmp = 0;
			switch (sortColumn) {
				case 'name': {
					const nameA = (a.ssid || a.mac || '').toLowerCase();
					const nameB = (b.ssid || b.mac || '').toLowerCase();
					cmp = nameA.localeCompare(nameB);
					break;
				}
				case 'type': {
					cmp = getDeviceType(a).localeCompare(getDeviceType(b));
					break;
				}
				case 'signal': {
					cmp = getDeviceSignal(a) - getDeviceSignal(b);
					break;
				}
				case 'channel': {
				...
				case 'last_seen': {
					const tsA = a.last_seen || a.last_time || 0;
					const tsB = b.last_seen || b.last_time || 0;
					cmp = tsA - tsB;
					break;
				}
			}
	...
	function processDevices(deviceList: KismetDevice[]) {
		...
		let _uncategorized = 0;

		deviceList.forEach((device) => {
			// Count WiFi devices
			if (
				device.type?.toLowerCase().includes('wi-fi') ||
				device.type?.toLowerCase().includes('wifi')
			) {
				_wifiDevices++;
			}
	...
	function getDeviceType(device: KismetDevice): string {
		...
		if (type.includes('client') || type === 'wi-fi client') {
			...
			// Mobile devices
			if (
				manuf.includes('apple') ||
				manuf.includes('samsung') ||
				manuf.includes('google') ||
				manuf.includes('oneplus') ||
				manuf.includes('xiaomi') ||
				manuf.includes('huawei') ||
				manuf.includes('motorola') ||
				manuf.includes('lg electronics') ||
				manuf.includes('nokia') ||
				manuf.includes('sony')
			) {
				return 'mobile';
			}
			...
			if (
				manuf.includes('dell') ||
				manuf.includes('hp') ||
				manuf.includes('lenovo') ||
				manuf.includes('asus') ||
				manuf.includes('acer') ||
				manuf.includes('microsoft') ||
				manuf.includes('intel corporate') ||
				manuf.includes('realtek') ||
				manuf.includes('broadcom') ||
				manuf.includes('qualcomm')
			) {
				return 'laptop';
			}
		...
		// IoT devices
		if (
			manuf.includes('amazon') ||
			manuf.includes('ring') ||
			manuf.includes('nest') ||
			manuf.includes('sonos') ||
			manuf.includes('roku') ||
			type.includes('iot') ||
			manuf.includes('espressif') ||
			manuf.includes('raspberry') ||
	...
	function isThreadDevice(device: KismetDevice): boolean {
		...
		const signal =
			typeof device.signal === 'object' ? device.signal?.last_signal : device.signal;
		if (device.type === 'Unknown' && signal && typeof signal === 'number' && signal < -30)
			return true;
	...
	function updateFilteredDevices() {
		let result: KismetDevice[];
		if (!selectedCategory) {
			result = devices;
		} else if (selectedCategory === 'drone') {
			result = devices.filter((device) => isDrone(device));
		} else {
		...
		filteredDevices = sortDevices(result);
	}
									...
									<div>
										<span
											...
											style="color: {getSignalColor(
												typeof device.signal === 'object'
													? device.signal?.last_signal || -80
													: device.signal || -80
											)}"
										>
											{typeof device.signal === 'object'
												? device.signal?.last_signal || 'N/A'
												: device.signal || 'N/A'} dBm
										</span>
									</div>
									<div>{device.channel || 'N/A'}</div>
									<div>
										{formatLastSeen(
											device.last_seen || device.last_time || Date.now()
										)}
									</div>
								</div>
							{/each}
				...
				<div class="detail-content">
					...
					<div class="detail-section">
						...
						<div class="detail-row">
							...
							<span class="detail-value"
								>{selectedDevice.manufacturer || 'Unknown'}</span
							>
						</div>
						<div class="detail-row">
					...
					<div class="detail-section">
						...
						<div class="detail-row">
							...
							<span
								class="detail-value"
								style="color: {getSignalColor(
									selectedDevice.signal?.last_signal || -80
								)}"
							>
						...
						{#if selectedDevice.signal?.max_signal}
							<div class="detail-row">
								...
								<span class="detail-value"
									>{selectedDevice.signal.max_signal} dBm</span
								>
							</div>
						{/if}
						{#if selectedDevice.signal?.min_signal}
							<div class="detail-row">
								...
								<span class="detail-value"
									>{selectedDevice.signal.min_signal} dBm</span
								>
							</div>
						{/if}
						<div class="detail-row">
					...
					<div class="detail-section">
						...
						<div class="detail-row">
							...
							<span class="detail-value"
								>{formatLastSeen(selectedDevice.last_seen || Date.now())}</span
							>
						</div>
						<div class="detail-row">
					...
					{#if selectedDevice.location}
						<div class="detail-section">
							...
							<div class="detail-row">
								...
								<span class="detail-value"
									>{selectedDevice.location.lat?.toFixed(6) || 'N/A'}</span
								>
							</div>
							<div class="detail-row">
								<span class="detail-label">Longitude:</span>
								<span class="detail-value"
									>{selectedDevice.location.lon?.toFixed(6) || 'N/A'}</span
								>
							</div>
						</div>
					{/if}
						...
						<div class="detail-row">
							...
							<span class="detail-value">
								{#if isDrone(selectedDevice)}
									🚁 Drone
								{:else}
									{getDeviceType(selectedDevice).charAt(0).toUpperCase() +
										getDeviceType(selectedDevice).slice(1)}
								{/if}
							</span>
						</div>
						<div class="detail-row">
							...
							<span
								class="detail-value {isThreadDevice(selectedDevice)
									? 'threat'
									: 'safe'}"
							>
								{isThreadDevice(selectedDevice)
									? '⚠️ Potential Threat'
									: '✅ Normal'}
							</span>
						</div>
