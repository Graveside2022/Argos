<script lang="ts">
	...
	interface Props {
		...
		isOpen?: boolean;
		onclose?: () => void;
		onselectsignal?: (signal: SignalMarker) => void;
	}

	let { cluster = null, isOpen = false, onclose, onselectsignal }: Props = $props();
	...
	let sortedSignals = $derived(
		cluster ? [...cluster.signals].sort((a, b) => b.power - a.power) : []
	);

	// Group signals by type
	let signalsByType = $derived.by(() => {
		return sortedSignals.reduce(
			(acc: Record<string, SignalMarker[]>, signal) => {
...
{#if isOpen && cluster}
	...
	>
		<div
			class="bg-gray-900 rounded-lg shadow-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden"
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-sm text-gray-400 mt-1">
							{cluster.stats.count} signals at {cluster.lat.toFixed(6)}°, {cluster.lon.toFixed(
								6
							)}°
						</p>
					</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
					...
					<div class="bg-gray-800 rounded-lg p-4">
						...
						<div class="text-white">
							{cluster.stats.minPower.toFixed(0)} to {cluster.stats.maxPower.toFixed(
								0
							)} dBm
						</div>
					</div>
