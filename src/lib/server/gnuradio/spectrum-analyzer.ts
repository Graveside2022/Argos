...
export class SpectrumAnalyzer extends EventEmitter {
	...
	async start(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to start spectrum analyzer: ${error instanceof Error ? (error as Error).message : String(error)}`,
			);
		}
	}
	...
	async stop(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to stop spectrum analyzer: ${error instanceof Error ? (error as Error).message : String(error)}`,
			);
		}
	}
