...
function decodeRR(bytes: number[], msgType: number): L3DecodedMessage {
	...
	switch (msgType) {
		...
		case 0x1b: {
			// SI3 contains Cell Identity and LAC
		...
		case 0x21: {
			...
			details.push('Paging for mobile stations');

			// Try to extract IMSI/TMSI if present
		...
		case 0x3e:
			details.push('Channel allocation');
			return {
...
function decodeMM(bytes: number[], msgType: number): L3DecodedMessage {
	...
	switch (msgType) {
		case 0x18: {
			...
			details.push(`Requesting: ${idName}`);
		...
		case 0x19: {
			// Identity Response - Contains IMSI/IMEI/TMSI
