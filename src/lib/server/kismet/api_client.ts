...
export class KismetAPIClient extends EventEmitter {
    ...
    async getDevices(): Promise<any[]> {
        try {
            ...
            // Method 1: Recent devices (last 30 minutes for better coverage)
            try {
                const timestamp = Math.floor(Date.now() / 1000) - 1800; // 30 minutes instead of 5
                const response = await this.makeRequest('GET', `/devices/last-time/${timestamp}/devices.json`);
            ...
            try {
                ...
                devices = Array.isArray(response) ? response.slice(0, 100) : [];
            ...
            // Method 3: All devices (with limit)
            try {
