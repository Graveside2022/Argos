...
export class DeviceIntelligence {
	...
	lookupManufacturer(mac: string): string {
		try {
			...
			logError("Error looking up manufacturer", {
				mac,
				error: (error as Error).message,
			});
			return "Unknown";
	...
	analyzeBehavior(mac: string, history: any[]): any {
		try {
			...
			logError("Error analyzing behavior", {
				mac,
				error: (error as Error).message,
			});
			return null;
		...
		try {
			return {
				...
				signalCharacteristics:
					this.analyzeSignalCharacteristics(device),
	...
	isAndroidDevice(device: any): boolean {
		...
		return androidPatterns.some(
			(pattern) => pattern.test(manufacturer) || pattern.test(deviceName),
		);
	}
	...
	isIoTDevice(device: any): boolean {
		...
		return iotPatterns.some(
			(pattern) =>
				pattern.test(manufacturer) ||
	...
	isEnterpriseDevice(device: any): boolean {
		...
		return enterprisePatterns.some(
			(pattern) => pattern.test(manufacturer) || pattern.test(deviceName),
		);
	}
	...
	private extractCapabilities(device: any): string[] {
		...
		if (device.capabilities?.includes("802.11n"))
			capabilities.push("802.11n");
		if (device.capabilities?.includes("802.11ac"))
			capabilities.push("802.11ac");
		if (device.capabilities?.includes("802.11ax"))
			capabilities.push("802.11ax");
	 ...
	 */
	private analyzeSignalPatterns(_history: any[]): any {
	 ...
	 */
	private analyzeMobilityPatterns(_history: any[]): any {
	 ...
	 */
	private analyzeActivityPatterns(_history: any[]): any {
	 ...
	 */
	private analyzeProbePatterns(_history: any[]): any {
	 ...
	 */
	private analyzeTemporalPatterns(_history: any[]): any {
