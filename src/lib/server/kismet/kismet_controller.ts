...
import { KismetAPIClient } from './api_client';
...
export class KismetController extends EventEmitter {
    ...
    private async startKismetServer(): Promise<void> {
        ...
        try {
            const kismetArgs = [
                '--tcp-port', this.config.restPort.toString(),
                '--no-line-wrap',
                '--no-ncurses',
                '--daemonize',
                '--silent'
     ...
     */
    private calculateThreatLevel(device: any, securityAssessment: any): 'low' | 'medium' | 'high' | 'critical' | 'unknown' {
