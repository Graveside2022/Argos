...
export class WiFiAdapterDetector {
    ...
    static async detectAllWiFiAdapters(): Promise<WiFiAdapter[]> {
        ...
        try {
            ...
            for (const iface of interfaces) {
                ...
                // Get MAC address
                try {
                    adapter.macAddress = (await readFile(join(ifacePath, 'address'), 'utf-8')).trim();
                } catch (e) {}
     ...
     */
    private static async getUSBInfo(ifacePath: string): Promise<{ usbId: string; description?: string; monitorModeCapable?: boolean } | null> {
        try {
    ...
    private static async checkMonitorModeSupport(iface: string): Promise<boolean> {
        try {
            // Try using iw
            const { stdout } = await execAsync(`iw phy $(cat /sys/class/net/${iface}/phy80211/name) info 2>/dev/null`);
    ...
    static async getBestMonitorInterface(): Promise<string | null> {
        ...
        if (usbAdapters.length > 0) {
            logInfo(`Selected USB WiFi adapter: ${usbAdapters[0].interface} (${usbAdapters[0].description || 'Unknown chipset'})`);
