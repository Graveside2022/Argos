...
export type { NetworkPacket } from '$lib/types/wireshark';
import type { NetworkPacket } from '$lib/types/wireshark';
...
export class WiresharkController extends EventEmitter {
	...
	async start(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to start Wireshark: ${error instanceof Error ? (error as Error).message : String(error)}`
			);
		}
	}
	...
	private async tryRealCapture(): Promise<boolean> {
		try {
			...
			const args = [
				...
				'-f',
				'ip', // Only capture IP traffic
				'-c',
				'10' // Capture 10 packets at a time for streaming
			];
			...
			await new Promise((resolve, reject) => {
				...
				this.process!.stderr?.once('data', (data) => {
					...
					if (
						data.toString().includes('permission') ||
						data.toString().includes('permitted')
					) {
						this.process!.kill();
						this.process = null;
						reject(new Error('Permission denied'));
	...
	async stop(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to stop Wireshark: ${error instanceof Error ? (error as Error).message : String(error)}`
			);
		}
	}
		...
		return new Promise((resolve, reject) => {
			...
			const checkNext = (index: number) => {
				if (index >= tools.length) {
					reject(
						new Error(
							'No packet capture tool found. Please install tshark, tcpdump, or dumpcap.'
						)
					);
					return;
				}
		...
		this.process.stdout?.on('data', (data) => {
			...
			if (expectingArray) {
				...
				if (arrayMatch) {
					try {
						console.log(
							'[SEARCH] Parsing JSON array:',
							arrayMatch[0].substring(0, 200)
						);
				...
				for (const line of lines) {
					...
					if (trimmed.startsWith('{') && trimmed.includes('"_source"')) {
						try {
							...
							console.log(
								'[SEARCH] Parsing individual JSON object:',
							...
							if (packet) {
								...
								this.emit('packet', packet);
								console.log(
									'[OK] Individual packet processed:',
									packet.src_ip,
									'->',
									packet.dst_ip
								);
	...
	private parsePacket(data: any): NetworkPacket | null {
		try {
			...
			if (!layers) {
				console.log(
					'[ERROR] No layers found in packet data:',
