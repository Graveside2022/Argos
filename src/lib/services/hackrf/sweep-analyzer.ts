...
import type { SpectrumData, SignalDetection } from '$lib/services/api/hackrf';
...
class SweepAnalyzer {
	...
	analyzeSpectrum(data: SpectrumData): void {
		if (!data.frequencies || !data.powers || data.frequencies.length !== data.powers.length) {
			return;
		}
		...
		// Detect peaks
		const peaks = this.detectPeaks(data.frequencies, data.powers, stats.noiseFloor);
	...
	private calculateStatistics(powers: number[]): {
		...
		const noiseIndex = Math.floor(
			(sortedPowers.length * this.options.noiseFloorPercentile) / 100
		);
	 ...
	 */
	private detectPeaks(frequencies: number[], powers: number[], noiseFloor: number): Peak[] {
		const peaks: Peak[] = [];
	...
	smoothPowers(powers: number[]): number[] {
		...
		for (let i = 1; i < powers.length - 1; i++) {
			smoothed[i] = powers[i] * (1 - factor) + ((powers[i - 1] + powers[i + 1]) * factor) / 2;
		}
