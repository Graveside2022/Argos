...
interface DeviceManagerState {
	...
	locationHistory: Map<
		string,
		Array<{ lat: number; lon: number; signal: number; timestamp: string }>
	>;
	lastUpdate: number;
}
...
class DeviceManager {
	...
	constructor() {
		this.devices = derived(this.state, ($state) => Array.from($state.devices.values()));
		...
		this.deviceStats = derived(this.state, ($state) => {
			...
			return {
				...
				active: devices.filter((d) => d.activity && d.activity.activityScore > 50).length
			};
		});
	}
	...
	setOptions(options: Partial<TrackingOptions>): void {
		...
		if (options.enableLocation !== undefined || options.locationUpdateInterval !== undefined) {
	...
	processDevice(device: KismetDevice): void {
		this.state.update((state) => {
			...
			if (this.options.enableLocation && device.gps) {
				this.updateLocation(
					enriched,
					device.gps as { lat: number; lon: number; alt?: number }
				);
			}
	...
	private updateMovement(device: EnrichedDevice): void {
		...
		// Calculate direction
		const direction = this.calculateBearing(first.lat, first.lon, last.lat, last.lon);
	...
	private updateSecurity(device: EnrichedDevice): void {
		...
		device.security = {
			encryption:
				device.encryption && Array.isArray(device.encryption) ? device.encryption : [],
