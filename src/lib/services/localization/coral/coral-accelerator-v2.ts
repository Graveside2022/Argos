...
export class CoralAccelerator extends EventEmitter {
	...
	private async startProcess(): Promise<void> {
		...
		const pythonExe = fs.existsSync(pythonPath)
			? pythonPath
			: fallbackPython;
	...
	private handleProcessExit(): void {
		...
		if (this.restartAttempts < this.MAX_RESTART_ATTEMPTS) {
			...
			console.log(
				`Attempting to restart Coral process (${this.restartAttempts}/${this.MAX_RESTART_ATTEMPTS})...`,
			);
			...
			console.error(
				"Max restart attempts reached. Coral acceleration disabled.",
			);
			this.emit(
				"fatal-error",
				new Error("Coral process failed to restart"),
			);
		}
	}
	...
	private startHealthCheck(): void {
		this.healthCheckInterval = setInterval(() => {
			...
			if (timeSinceLastCheck > 60000) {
				// 1 minute without health response
 ...
 */
export async function createCoralAccelerator(
	modelPath?: string,
): Promise<CoralAccelerator | null> {
	try {
		...
		console.warn(
			"[WARN] Coral TPU not available, will use CPU fallback:",
			error,
		);
		return null;
	}
