...
function createGrid(
	...
	const lonMetersPerDegree =
		111320 * Math.cos((((bounds.minLat + bounds.maxLat) / 2) * Math.PI) / 180);
		...
		while (found) {
			...
			for (let j = 0; j < segments.length; j++) {
				...
				const dist1 = Math.hypot(
					lastPoint[0] - segment[0][0],
					lastPoint[1] - segment[0][1]
				);
				const dist2 = Math.hypot(
					lastPoint[0] - segment[1][0],
					lastPoint[1] - segment[1][1]
				);
...
function smoothPolyline(points: [number, number][], iterations: number): [number, number][] {
	...
	for (let i = 0; i < iterations; i++) {
		...
		for (let j = 0; j < smoothed.length - 1; j++) {
			...
			newPoints.push([0.75 * p1[0] + 0.25 * p2[0], 0.75 * p1[1] + 0.25 * p2[1]]);
			newPoints.push([0.25 * p1[0] + 0.75 * p2[0], 0.25 * p1[1] + 0.75 * p2[1]]);
		}
