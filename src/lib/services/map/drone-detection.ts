...
import {
	...
	DRONE_FREQUENCY_BANDS as _DRONE_FREQUENCY_BANDS
...
export class DroneDetectionService {
		...
		signals.forEach((signal) => {
			...
			unassigned.forEach((other) => {
				if (this.areSignalsRelated(signal, other)) {
					...
					group.centerLat =
						(group.centerLat * (group.signals.length - 1) + other.lat) /
						group.signals.length;
					group.centerLon =
						(group.centerLon * (group.signals.length - 1) + other.lon) /
						group.signals.length;
		...
		groups.forEach((group, i) => {
			...
			groups.forEach((other, j) => {
				...
				if (distance < 200 && timeOverlap) {
					...
					mergedGroup.centerLat =
						mergedGroup.signals.reduce((sum, s) => sum + s.lat, 0) / totalSignals;
					mergedGroup.centerLon =
						mergedGroup.signals.reduce((sum, s) => sum + s.lon, 0) / totalSignals;

					// Update time range
					mergedGroup.timeRange.start = Math.min(
						mergedGroup.timeRange.start,
						other.timeRange.start
					);
					mergedGroup.timeRange.end = Math.max(
						mergedGroup.timeRange.end,
						other.timeRange.end
					);
				}
			});
	...
	private determineDroneType(signals: SignalMarker[]): DroneSignature['type'] {
		...
		const hasVideo = Array.from(uniqueFreqs).some(
			(f) => (f >= 1200 && f <= 1300) || (f >= 2400 && f <= 2500) || (f >= 5725 && f <= 5875)
		);

		// Check for control frequencies
		const hasControl = Array.from(uniqueFreqs).some(
			(f) => (f >= 2400 && f <= 2483) || (f >= 433 && f <= 435) || (f >= 900 && f <= 928)
		);

		// Check for telemetry (often on 433 or 915 MHz)
		const hasTelemetry = Array.from(uniqueFreqs).some(
			(f) => (f >= 433 && f <= 435) || (f >= 902 && f <= 928)
		);
	...
	private analyzeDroneCharacteristics(signals: SignalMarker[]): DroneCharacteristics {
		...
		for (const [mfr, pattern] of Object.entries(DRONE_PATTERNS)) {
			...
			const matchesPower =
				avgPower >= pattern.powerRange[0] && avgPower <= pattern.powerRange[1];
	...
	private updateTrajectoryMetrics(trajectory: TrajectoryData): void {
		...
		if (recent.length >= 2) {
			...
			trajectory.heading = headings.length > 0 ? this.averageHeading(headings) : 0;
	...
	private generateAlerts(): DroneAlert[] {
		...
		this.activeDrones.forEach((drone) => {
			...
			if (
				drone.characteristics.powerProfile === 'military' ||
	...
	private generateStatistics(): DroneStatistics {
		...
		stats.avgConfidence =
			this.activeDrones.size > 0 ? totalConfidence / this.activeDrones.size : 0;

		// Calculate detection rate (detections per minute)
		const historyCount = Array.from(this.detectionHistory.values()).reduce(
