...
// Re-export canonical types from $lib/types/network (Phase 0.6.2 backward compat)
export type { NetworkNode, NetworkEdge } from '$lib/types/network';
...
function detectRelationship(
	...
	const distance = calculateDistance(signal1.lat, signal1.lon, signal2.lat, signal2.lon);
	...
	if (
		signal1.metadata?.signalType === 'bluetooth' &&
		signal2.metadata?.signalType === 'bluetooth'
	) {
...
function exploreCluster(
	...
	clusterNodes.forEach((nodeId) => {
		...
		const dist = calculateDistance(centerLat, centerLon, node.signal.lat, node.signal.lon);
