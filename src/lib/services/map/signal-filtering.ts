...
export class SignalFilterService {
	...
	private filterBySignalStrength(signals: SignalMarker[]): SignalMarker[] {
		...
		return signals.filter((signal) => signal.power >= minPower && signal.power <= maxPower);
	}
	...
	private filterByFrequencyBands(signals: SignalMarker[]): SignalMarker[] {
		...
		return signals.filter((signal) => {
			...
			const isExcluded = excludeList.some(
				(band) => freq >= band.minFreq && freq <= band.maxFreq
			);
			...
			return includeBands.some((band) => freq >= band.minFreq && freq <= band.maxFreq);
		});
	}
	...
	private filterByTemporalPatterns(signals: SignalMarker[]): SignalMarker[] {
		...
		signals.forEach((signal) => {
			...
			const recentHistory = history.filter((s) => now - s.timestamp <= timeWindow);
	...
	private prioritizeSignals(signals: SignalMarker[]): SignalMarker[] {
		...
		return scoredSignals.slice(0, maxSignals).map((item) => item.signal);
	}
	...
	private calculateMovement(history: SignalMarker[]): MovementData {
		...
		const distance = this.calculateDistance(previous.lat, previous.lon, recent.lat, recent.lon);

		const timeDelta = (recent.timestamp - previous.timestamp) / 1000; // seconds
		const speed = timeDelta > 0 ? distance / timeDelta : 0;

		const direction =
			(Math.atan2(recent.lon - previous.lon, recent.lat - previous.lat) * 180) / Math.PI;
	...
	private getFrequencyBand(frequency: number): FrequencyBand | null {
		...
		return (
			allBands.find((band) => frequency >= band.minFreq && frequency <= band.maxFreq) || null
		);
	}
