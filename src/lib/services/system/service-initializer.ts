...
import { dataStreamManager } from "$lib/services/websocket/data-stream-manager";
...
export async function initializeServices(
	...
	try {
		...
		if (enableMonitoring) {
			logInfo(
				"Initializing system health monitor",
				{},
				"system-health-monitor-init",
			);
		...
		if (enableStreaming) {
			logInfo(
				"Initializing data stream manager",
				{},
				"data-stream-manager-init",
			);
...
export function shutdownServices(): void {
	...
	try {
		...
		logInfo(
			"Shutting down data stream manager",
			{},
			"data-stream-manager-shutdown",
		);
		...
		logInfo(
			...
			{},
			"system-health-monitor-shutdown",
		);
		...
		logInfo(
			...
			{},
			"error-recovery-service-shutdown",
		);
