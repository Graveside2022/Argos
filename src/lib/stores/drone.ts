...
export const missionProgress = derived([activeMission, droneState], ([$mission, $state]) => {
...
export function completeMission() {
	activeMission.update((mission) => {
		if (mission) {
			...
			// Add to history (cap at 50 missions)
			missionHistory.update((history) => [...history, mission].slice(-50));
...
const MAX_FLIGHT_POINTS = 10000;

export function recordFlightPoint(point: FlightPoint) {
	flightRecorder.update((state) => {
		if (state.recording) {
			state.points.push(point);
			if (state.points.length > MAX_FLIGHT_POINTS) {
				state.points = state.points.slice(-MAX_FLIGHT_POINTS);
			}
	...
	activeMission.update((mission) => {
		if (mission && mission.status === 'active') {
			...
			if (mission.flightPath.length > MAX_FLIGHT_POINTS) {
				mission.flightPath = mission.flightPath.slice(-MAX_FLIGHT_POINTS);
			}
			...
			if (mission.flightPath.length > 1) {
				...
				mission.statistics.maxAltitude = Math.max(
					mission.statistics.maxAltitude,
					point.altitude
				);
				mission.statistics.minAltitude = Math.min(
					mission.statistics.minAltitude,
					point.altitude
				);
			}
		}
