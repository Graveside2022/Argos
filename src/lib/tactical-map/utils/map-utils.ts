...
export function estimateDistanceFromRSSI(rssi: number, frequencyMHz?: number): number {
	// Reference RSSI at 1 meter: -40 dBm for 2.4GHz, -47 dBm for 5GHz
	const rssiRef = frequencyMHz && frequencyMHz > 3000 ? -47 : -40;
	// Path loss exponent: 3.0 for mixed indoor/outdoor
	const n = 3.0;

	const distance = Math.pow(10, (rssiRef - rssi) / (10 * n));
	return Math.max(1, Math.min(500, distance));
}

// Format RSSI-based distance estimate for display
export function formatDistanceEstimate(meters: number): string {
	let label: string;
	if (meters < 5) label = 'Very Close';
	else if (meters < 15) label = 'Close';
	else if (meters < 40) label = 'Nearby';
	else if (meters < 100) label = 'Moderate';
	else label = 'Far';

	if (meters < 1000) {
		return `~${Math.round(meters)}m (${label})`;
	}
	return `~${(meters / 1000).toFixed(1)}km (${label})`;
}
