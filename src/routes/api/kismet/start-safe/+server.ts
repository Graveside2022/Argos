...
export async function POST() {
	try {
		...
		await execAsync(
			'sudo cp ./scripts/kismet-no-auto-source.conf /etc/kismet/kismet_site.conf'
		);

		// Reset the wireless adapter
		const adapterCheck = await execAsync(
			"ip link show | grep -E 'wlx[0-9a-f]{10}' | cut -d: -f2 | tr -d ' ' | head -1"
		);
