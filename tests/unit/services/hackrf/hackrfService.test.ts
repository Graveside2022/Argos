...
vi.mock('$lib/services/websocket/hackrf', () => ({
	HackRFWebSocketClient: vi.fn().mockImplementation(() => ({
		on: vi.fn(),
		onMessage: vi.fn(),
		connect: vi.fn(),
		disconnect: vi.fn(),
...
describe('HackRFService - Core SDR Functionality', () => {
	...
	beforeEach(() => {
		...
		// Restore default API mock returns after clearAllMocks
		vi.mocked(hackrfAPI.connect).mockResolvedValue({ success: true, message: 'Connected' });
		vi.mocked(hackrfAPI.disconnect).mockResolvedValue({
			success: true,
			message: 'Disconnected'
		});
		vi.mocked(hackrfAPI.getStatus).mockResolvedValue({ connected: true, sweeping: false });
		vi.mocked(hackrfAPI.getConfig).mockResolvedValue({
			startFreq: 88,
			endFreq: 108,
			binSize: 100000,
			sampleRate: 20000000,
		...
		vi.mocked(hackrfAPI.updateConfig).mockResolvedValue({
			success: true,
			config: {
				startFreq: 88,
				endFreq: 108,
		...
		vi.mocked(hackrfAPI.startSweep).mockResolvedValue({
			success: true,
			message: 'Sweep started'
		});
		vi.mocked(hackrfAPI.stopSweep).mockResolvedValue({
			success: true,
	...
	describe('Device Connection Management', () => {
		...
		it('should handle connection failures gracefully', async () => {
			...
			vi.mocked(hackrfAPI.connect).mockRejectedValueOnce(new Error('Connection refused'));
		...
		it('should disconnect cleanly from device', async () => {
			...
			// After disconnect, getStatus should return disconnected state
			vi.mocked(hackrfAPI.getStatus).mockResolvedValueOnce({
				connected: false,
				sweeping: false
			});
			await hackrfService.disconnect();
	...
	describe('Service Integration', () => {
		...
		it('should handle service disconnection gracefully', async () => {
			// Disconnect the service — after disconnect, status should show disconnected
			vi.mocked(hackrfAPI.getStatus).mockResolvedValueOnce({
				connected: false,
				sweeping: false
			});
