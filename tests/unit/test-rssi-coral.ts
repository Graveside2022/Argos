...
// import { HybridRSSILocalizer } from '../../src/lib/services/localization/hybrid-rssi-localizer'; // Module removed
class HybridRSSILocalizer {
	async initialize() {}
	addMeasurement(..._args: any[]) {}
	async localize() {
		return { position: { lat: 0, lon: 0 }, confidence: 0, uncertaintyRadius: 0 };
	}
	async predictForDevice() {
		return { position: { lat: 0, lon: 0 }, confidence: 0, uncertaintyRadius: 0 };
	}
	async estimateSourceLocation() {
		return { position: { lat: 0, lon: 0 }, confidence: 0, uncertaintyRadius: 0 };
	}
	getStats() {
		return { coralUsage: 0, avgProcessingTime: 0 };
	}
	async shutdown() {}
}
...
async function testCoralIntegration() {
	...
	try {
		...
		console.log(
			`   Grid size: ${Math.sqrt(prediction.mean.length)}x${Math.sqrt(prediction.mean.length)}`
		);
		console.log(
			`   Mean RSSI range: ${Math.min(...prediction.mean).toFixed(1)} to ${Math.max(...prediction.mean).toFixed(1)} dBm\n`
		);
